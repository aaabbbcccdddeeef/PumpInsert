...
#define _MaxStepCount(a)  g_stPumpSetting[a].MaxStepCount//台阶数
#define _FullStepPerClass(a)   g_stPumpSetting[a].FullStepPerClass  //每个台阶的整步数 ,加速度
...
void PumpDetectRun(u8 PumpSel, u8 direction, u16 cntRun) //检测光耦信号，按方向和脉冲数运行
{
    ...
    if (cntRun > 2 * _MaxStepCount(PumpSel-1) * _FullStepPerClass(PumpSel-1) * _nStep2Pulse(PumpSel-1)) //超过加减速脉冲数
    {
    ...
    else if (cntRun <= 2 * 1 * _FullStepPerClass(PumpSel-1) * _nStep2Pulse(PumpSel-1)) //不超过第一个台阶
    {
        nMaxEffClass = 1;
        if(PumpGetOpticStatus(PumpSel) != _OPTIC_ON(PumpSel-1))
...
void PumpFreeRun(u8 PumpSel, u8 direction, u16 cntRun) //不管光耦信号，按方向和脉冲数运行
{
    ...
    else if ((cntRun <= 2 * 1 * _FullStepPerClass(PumpSel-1) * _nStep2Pulse(PumpSel-1)) && (cntRun > 0)) //不超过第一个台阶
    {
        nMaxEffClass = 1;
