...
uint32_t PWM_Init( uint32_t ChannelNum, uint32_t cycle )
{
  ...
  {
	//match_counter1 = 0;
	//PINCON->PINSEL7 = (3<<18);	/* set GPIOs for all PWM pins on PWM0 */

	PWM1->TCR = TCR_RESET;	/* Counter Reset */ 
	PWM1->PR = 0x00;		/* count frequency:Fpclk */
	PWM1->MCR = PWMMR0R;	/* interrupt on PWMMR0, reset on PWMMR0, reset 
							TC if PWM0 matches */				
...
void PWM_Start( uint32_t channelNum )
{
  if ( channelNum == 1 )
  {
	...
	PWM1->PCR = PWMENA2;//PWMENA1 | PWMENA2 | PWMENA3 | PWMENA4 | PWMENA5 | PWMENA6;
